...
import com.mclegoman.viewpoint.client.config.PerspectiveConfigDataLoader;
...
public class PerspectiveZoom {
	...
	public static float getZoomMultiplier() {
		return isZooming() ? (float) (1.0F - (Math.log(getZoomLevel() + 1.0F) / Math.log(100.0 + 1.0F))) : 1;
	}
	public static double limitFov(double fov) {
		return MathHelper.clamp(fov, 0.1F, 179.9F);
	}
