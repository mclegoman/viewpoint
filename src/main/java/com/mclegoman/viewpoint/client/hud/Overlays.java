...
public class Overlays {
	...
	public static void renderOverlays(DrawContext context) {
		if (!ClientData.minecraft.getDebugHud().shouldShowDebugHud() && !ClientData.minecraft.options.hudHidden && !HUDHelper.shouldHideHUD()) {
			...
			if (!ConfigHelper.getConfig("time_overlay").equals("false")) {
				if (ClientData.minecraft.world != null) {
					...
					String hour = ConfigHelper.getConfig("time_overlay").equals("twelve_hour") ? String.valueOf(rawHour == 0 || rawHour == 12 ? 12 : rawHour % 12) : String.valueOf(rawHour);
					if (rawHour < 10 && rawHour != 0) hour = "0" + hour;
					Text timePeriod = ConfigHelper.getConfig("time_overlay").equals("twelve_hour") ? (rawHour < 12 ? Translation.getTranslation(Data.version.getID(), "time_overlay.am") : Translation.getTranslation(Data.version.getID(), "time_overlay.pm")) : Text.empty();
