...
vec4 outline(vec4 color, sampler2D DepthSampler) {
    ...
    float outlineDepth = 2.0 * 0.025 * 1000.0 /
    (1000.0 + 0.025 - (depth * 2.0 - 1.0) * (1000.0 - 0.025));
    float offset = max(
    Thickness * max((32.0 - outlineDepth) / 32.0, 0.0),
    1.0 / OutSize.y
    );
    ...
    2.0 * 0.025 * 1000.0 /
    (1000.0 + 0.025 -
    (max(depth0, max(depth1, max(depth2, depth3))) * 2.0 - 1.0) *
    (1000.0 - 0.025)) -
    outlineDepth,
    0.0
    ) * 0.15,
    ...
    0.0, 1.0) * exp(-outlineDepth * 0.025);
    ...
    return vec4(
    mix(color.rgb, pow((pow(outlineColor, OutlinePow) * OutlineColorMultiplier) + Transparency, vec3(2.0)), amount),
    color.a
    );
}
...
void try_insert(vec4 color, sampler2D DepthSampler) {
    ...
    if (Silhouette != 0) {
        color = vec4(mix(color.rgb, SilhouetteColor, 1.0), 1.0);
    }
